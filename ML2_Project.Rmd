...
We reset factor levels for later visualization purposes for variable bimas, varieties, etc.
...
## standardizing variables due to correlation
**correlation with size**
the amount of seed, phosphor, urea, totlabour and pesticide is highly dependent on the size of the cultivated area. Thus we will calculate a ratio for each of these variables so that they are becoming independent. 
...
**correlation with labour**
create variable fam_ratio based on the amount of family labour from total labour
...
```{r}
res <- cor(RiceFarms[,-c(1,3:15,19)])
ggcorrplot(round(res, 2))
```

## Feature Engineering

wage variable shows a bimodal distribution. We will use this to create a categorical variable.
...
## Model split
60-20-20 split using seed to obtain always the same split
...
## Model metrics
**MSE** Mean Squared Error: comparing true values with predicted ones
```{r}
...
```
```{r}
gam5.2 <-  gam(log(goutput)~
                 s(log(size))+
                 s(log(totlab_size))+
                 s(log(urea_size), df=2), data = RiceFarms[train,])
#deviance(gam5)
#deviance(gam5.2) ## TODO: Check with others if we should use this

#par(mfrow=c(1,3))
#plot.Gam(gam5, residuals = TRUE, col="lightblue")
#plot.Gam(gam5.2, residuals = TRUE, col="lightblue")

#slope at beginning of urea size => set df to 2
```
...
gam6 <- gam(log(goutput)~
              s(log(size))+
              s(log(totlab_size))+
              s(log(urea_size), df=2)+
              s(log(phosph_size+1)), data = RiceFarms[train,])

gam6.1 <- gam(log(goutput)~
                s(log(size))+
                s(log(totlab_size))+
                s(log(urea_size), df=1) +
                s(log(phosph_size+1), df=2), data = RiceFarms[train,])

gam7 <- gam(log(goutput)~
              s(log(size))+
              s(log(totlab_size))+
              s(log(urea_size), df=2)+
...
gam10.1 <- gam(log(goutput)~
               ...
               s(log(phosph_size+1))+
               s(seed_size)+
               s(pest_size, df=2), data = RiceFarms[train,])
...
gams <- list(gam17,gam16, gam15, gam14, gam13, gam12.2, gam12, gam11, gam10.1, gam10, gam9, gam8 , gam7, gam6.1, gam6, gam5.2, gam5.1, gam5, gam4, gam3, gam2, gam1)
gams <- rev(gams)
...
kbl(model_perf, caption = "gam comparison") %>%
  kable_paper("hover", full_width = FALSE) %>%
  pack_rows("size", 1, 2) %>%
  pack_rows("labour", 3, 4) %>%
  pack_rows("urea", 5, 7) %>%
  pack_rows("phosphor", 8, 10) %>%
  pack_rows("seed", 11, 12) %>%
  pack_rows("pesticide", 13, 14) %>%
  pack_rows("price", 15, 15) %>%
  pack_rows("wage", 16, 17) %>%
  pack_rows("family labour", 18, 18) %>%
  pack_rows("categorical variables", 19, 22) %>%
  landscape()
...
```{r}
gams <- list(gam17, gam14, gam13,gam12.2, gam11, gam10, gam9, gam6, gam5, gam3, gam2)
gams <- rev(gams)
...
```{r}
par(mfrow=c(3,4))
plot.Gam(gam17, residuals = TRUE, se=TRUE, col="lightblue")
```
